                  ...
                  {data.recent_orders.map((order) => (
                    <div key={order.id} className="p-4 hover:bg-slate-50 transition-colors">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <span className="text-sm font-bold text-slate-900 tabular-nums tracking-tight">{formatINR(order.total_amount)}</span>
                      </div>
                    </div>
                  ))}
                </div>
                    ...
                    return (
                      <div key={row.order_id} className="p-4 hover:bg-slate-50 transition-colors">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-bold text-slate-900 tabular-nums tracking-tight">{formatINR(row.total_amount)}</span>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {data?.top_products?.map((p, i) => (
                      <tr key={p.product_name} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="py-3 text-right font-bold text-slate-900 tabular-nums tracking-tight">{formatINR(p.total_spend)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
